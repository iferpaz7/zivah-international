...
export const quoteFormSchema = z.object({
  ...
  message: z.string()
    .optional()
    .refine((val) => !val || (val.length <= 1000 && !/<script/i.test(val)), 'Message is too long or contains invalid content'),

  items: z.array(z.object({
    productId: z.number().int().positive('Invalid product'),
    measureId: z.number().int().positive('Invalid measure').optional(),
    quantity: z.number().int().min(1, 'Quantity must be at least 1').max(10000, 'Quantity is too large'),
    unitPrice: z.number().min(0, 'Price cannot be negative').max(1000000, 'Price is too high').optional(),
...
export class FormRateLimiter {
  private submissions = new Map<string, { count: number; resetTime: number; lastSubmission: number }>()
  ...
  canSubmit(identifier: string): { allowed: boolean; waitTime?: number; reason?: string } {
    const now = Date.now()
    ...
    if (now - record.lastSubmission < this.cooldownMs) {
      ...
      return { allowed: false, waitTime, reason: 'Please wait before submitting again' }
    }
    ...
    if (record.count >= this.maxSubmissions) {
      ...
      return { allowed: false, waitTime, reason: 'Too many submissions. Please try again later' }
    }
