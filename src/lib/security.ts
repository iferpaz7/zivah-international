...
class SimpleRateLimiter {
  ...
  check(key: string): { success: boolean; remaining: number; reset: number } {
    ...
    record.count++
    return { success: true, remaining: this.limit - record.count, reset: record.resetTime }
  }
}
...
export function validateInput(input: string, type: 'email' | 'text' | 'url' | 'phone' | 'name'): boolean {
  const patterns = {
    email: /^[^\s@]+@[^\s@]+\.[^\s@]+$/,
    text: /^[a-zA-Z0-9\s\-_.,!?()]+$/,
